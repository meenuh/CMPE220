...
char *convertInstrToBin(char *instr, int currMemLoc) {
    ...
    if (strcmp(tokens[0], "lw") == 0) {
        ...
        strcpy(binInstr, JR);
        //we need to pad the tokens for R type. easiest way to do this
        tokens[2] = malloc(3);
        tokens[3] = malloc(3);
        tokens[2][2] = '\0';
        tokens[3][2] = '\0';
...
char *subBinary(char *opLeft, char *opRight, int size, int setFlags){
    ...
    right = addBinary(decimalToBinary(1, size), compl, size, 0);
    free(compl);
    ...
    while(i < info.lines){
        ...
        if (strncmp(LW, instr, OPCODE_SIZE) == 0) { //LOAD WORD
           ...
           // if(binaryToDecimal(regFile[leftReg], WORD_SIZE) == binaryToDecimal(regFile[rightReg], WORD_SIZE)){
            ...
            char *temp = ALU(ADD_OP, PC, "1", WORD_SIZE, 0);
            while(strcmp(instr, "00000000000000000000000000000000") == 0) {
                free(temp);
                temp = ALU(ADD_OP, PC, "1", WORD_SIZE, 0);
            }

            strcpy(regFile[6], temp);
            free(temp);
